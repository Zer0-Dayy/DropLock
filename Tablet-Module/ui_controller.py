...
def _run_ui_process(ui_queue: mp.Queue, fullscreen: bool) -> None:
    ...
    header = tk.Frame(wrapper, bg="#0b1220", height=96)
    header.pack(fill="x")
    header.pack_propagate(False)

    brand = tk.Label(header, text="DropLock", fg="#f8fafc", bg="#0b1220", font=("Helvetica", 36, "bold"))
    brand.pack(side="left", padx=24, pady=16)

    subtitle_small_var = tk.StringVar(value="Sector Hub")
    subtitle_small = tk.Label(header, textvariable=subtitle_small_var, fg="#60a5fa", bg="#0b1220", font=("Helvetica", 16))
    subtitle_small.pack(side="left", padx=(0, 12), pady=18)
    ...
    body = tk.Frame(wrapper, bg="#020617", padx=30, pady=20)
    ...
    status_badge = tk.Label(body, textvariable=state_var, fg="#e2e8f0", bg=STATE_COLORS["idle"], font=("Helvetica", 16, "bold"), padx=16, pady=8)
    status_badge.pack(anchor="w", pady=(4, 20))

    title_label = tk.Label(body, textvariable=title_var, fg="#f8fafc", bg="#020617", font=("Helvetica", 50, "bold"), anchor="w", justify="left", wraplength=920)
    title_label.pack(fill="x")

    msg_label = tk.Label(body, textvariable=msg_var, fg="#cbd5e1", bg="#020617", font=("Helvetica", 26), anchor="w", justify="left", wraplength=920, pady=10)
    msg_label.pack(fill="x")

    last_update = tk.Label(body, textvariable=updated_var, fg="#64748b", bg="#020617", font=("Helvetica", 14), anchor="w")
    last_update.pack(fill="x", pady=(8, 0))

    footer = tk.Frame(wrapper, bg="#0b1220", height=124)
    footer.pack(fill="x")
    ...
    guidance.pack(anchor="w", padx=24, pady=(18, 8))
