...
class SessionOrchestrator:
    ...
    def _resolve_tamper_recipients(self) -> list[dict[str, str]]:
        ...
        try:
            ...
            admin_uids = [
                uid for uid, enabled in (sector.get("localAdminUids") or {}).items() if self._coerce_bool(enabled)
            ]
        except Exception:
        ...
        # Fallback for environments where sectors/{sectorId}/localAdminUids is unavailable or empty.
        try:
        ...
        for profile in profiles.values():
            ...
            if role == "admin" and sector_id != self._device_context.sector_id:
                continue
