...
def calibrate_weight_sensors_at_startup(locker_manager: LockerManager):
    logger.info("Starting weight sensor calibration (ensure lockers are empty)")
    results = locker_manager.calibrate_weight_sensors()
    for locker_id, ok in results.items():
        if ok:
            logger.info("Calibration successful for %s", locker_id)
        else:
            logger.warning("Calibration failed for %s; using default offset", locker_id)
...
def main() -> int:
    ...
    calibrate_weight_sensors_at_startup(locker_manager)
